      ...
      <Container className="relative text-center">
        ...
        <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {LIFESTYLE_MOMENTS.map((m) => (
            <div key={m.label} className="rounded-sm border border-white/10 bg-lf-charcoal/60 p-6">
              <p className="font-kicker text-sm uppercase tracking-wide2 text-lf-gold">{m.label}</p>
              <p className="mt-3 text-sm text-lf-cream/80">{m.copy}</p>
            </div>
          ))}
        </div>
      </Container>
